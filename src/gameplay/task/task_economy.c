...
bool Tasks_IsNearWorkbench(const Player *player) {
    ...
    workbenchX = WORKBENCH_X;
    workbenchY = WORKBENCH_Y;
    Map_GetRuntimeAnchorPosition(MAP_ANCHOR_WORKBENCH, &workbenchX, &workbenchY);
    return TasksRuntime_IsNearRect(player, workbenchX, workbenchY, STATION_FOOTPRINT_WIDTH, STATION_FOOTPRINT_HEIGHT);
}
