...
void SaveInternal_SanitizeSnapshot(SaveSnapshot *snapshot) {
    ...
    if (snapshot->mapWidth <= 0 || snapshot->mapHeight <= 0) {
        snapshot->mapWidth = MAP_WIDTH;
        snapshot->mapHeight = MAP_HEIGHT;
    }
    ...
    snapshot->mapStateCount = ClampInt(snapshot->mapStateCount, 0, SAVE_MAP_STATE_MAX);
    for (int mapIndex = 0; mapIndex < snapshot->mapStateCount; mapIndex++) {
        SavedMapStateSnapshot *state = &snapshot->mapStates[mapIndex];
        state->contentVersion = state->contentVersion > 0 ? state->contentVersion : 1;
        state->unlockCount = ClampInt(state->unlockCount, 0, MAX_MAP_UNLOCKS);
        state->clearedDynamicTileCount = ClampInt(state->clearedDynamicTileCount, 0, SAVE_DYNAMIC_TILE_MAX);
        state->nodeCount = ClampInt(state->nodeCount, 0, MAX_RESOURCE_NODES);
        state->monsterCount = ClampInt(state->monsterCount, 0, MAX_MONSTERS);
        state->logCount = ClampInt(state->logCount, 0, MAX_LOGS);
        for (int nodeIndex = 0; nodeIndex < state->nodeCount; nodeIndex++) {
            if (state->nodes[nodeIndex].respawnsRemaining < 0) state->nodes[nodeIndex].respawnsRemaining = 0;
        }
        for (int monsterIndex = 0; monsterIndex < state->monsterCount; monsterIndex++) {
            SavedMonsterSnapshot *monster = &state->monsters[monsterIndex];
            monster->health = SanitizeFiniteFloat(monster->health, 0.0f, 0.0f, 500.0f);
            if (monster->health <= 0.0f) monster->active = false;
        }
        if (!state->campPlaced) {
            state->campX = -1;
            state->campY = -1;
...
bool SaveInternal_IsSnapshotBinaryDataValid(const SaveSnapshot *snapshot) {
    ...
    for (int mapIndex = 0; mapIndex < snapshot->mapStateCount && mapIndex < SAVE_MAP_STATE_MAX; mapIndex++) {
        const SavedMapStateSnapshot *state = &snapshot->mapStates[mapIndex];
        for (int monsterIndex = 0; monsterIndex < state->monsterCount && monsterIndex < MAX_MONSTERS; monsterIndex++) {
            if (!IsFiniteFloat(state->monsters[monsterIndex].health)) return false;
        }
    }
