...
#if defined(__APPLE__)
    {
        const fs::path bundleResourceCandidate = executableDir.parent_path() / "Resources" / relativePath;
        if (PathExists(bundleResourceCandidate)) {
            return CopyResolvedPath(buffer, bufferSize, bundleResourceCandidate);
        }
    }
#endif
