...
Rectangle UI_GetMainMenuButtonRect(int screenWidth, int screenHeight, int buttonIndex) {
    ...
    width = 348.0f * scale;
    height = 62.0f * scale;
    startY = screenHeight * 0.455f;
    centerX = screenWidth * 0.205f;

    return Rectangle{
        centerX - width * 0.5f,
        startY + buttonIndex * (height + 15.0f * scale),
        width,
        height
    };
}
...
Rectangle UI_GetAuthPanelRect(int screenWidth, int screenHeight) {
    ...
    scale = UIRuntime_GetScale(screenWidth, screenHeight);
    return Rectangle{
        screenWidth * 0.5f - 530.0f * scale,
        screenHeight * 0.5f - 295.0f * scale,
        1060.0f * scale,
        590.0f * scale
    };
}
...
Rectangle UI_GetAuthInputRect(int screenWidth, int screenHeight, int fieldIndex) {
    ...
    return Rectangle{
        panel.x + 430.0f * scale,
        panel.y + 188.0f * scale + fieldIndex * 94.0f * scale,
        panel.width - 480.0f * scale,
        68.0f * scale
    };
}
...
Rectangle UI_GetAuthPasswordToggleRect(int screenWidth, int screenHeight) {
    ...
    return Rectangle{
        passwordRect.x + passwordRect.width - 126.0f * scale,
        passwordRect.y + 14.0f * scale,
        108.0f * scale,
        40.0f * scale
    };
}
...
Rectangle UI_GetAuthDeleteAccountRect(int screenWidth, int screenHeight) {
    ...
    return Rectangle{
        panel.x + 430.0f * scale,
        panel.y + panel.height - 68.0f * scale,
        208.0f * scale,
        40.0f * scale
    };
}
...
Rectangle UI_GetAuthSubmitButtonRect(int screenWidth, int screenHeight) {
    ...
    return Rectangle{
        panel.x + 430.0f * scale,
        panel.y + panel.height - 136.0f * scale,
        268.0f * scale,
        58.0f * scale
    };
}
...
Rectangle UI_GetAuthExitButtonRect(int screenWidth, int screenHeight) {
    ...
    panel = UI_GetAuthPanelRect(screenWidth, screenHeight);
    return Rectangle{
        panel.x + panel.width - 154.0f * scale,
        panel.y + 34.0f * scale,
        112.0f * scale,
        42.0f * scale
    };
...
Rectangle UI_GetAuthAccountPickerPanelRect(int screenWidth, int screenHeight) {
    ...
    scale = UIRuntime_GetScale(screenWidth, screenHeight);
    return Rectangle{
        screenWidth * 0.5f - 300.0f * scale,
        screenHeight * 0.5f - 245.0f * scale,
        600.0f * scale,
        490.0f * scale
    };
}
...
Rectangle UI_GetAuthAccountPickerRowRect(int screenWidth, int screenHeight, int visibleIndex) {
    ...
    return Rectangle{
        panel.x + 36.0f * scale,
        panel.y + 104.0f * scale + visibleIndex * 54.0f * scale,
        panel.width - 72.0f * scale,
        44.0f * scale
    };
}
...
Rectangle UI_GetPauseMenuPanelRect(int screenWidth, int screenHeight) {
    ...
    return Rectangle{
        firstButton.x - 70.0f * scale,
        firstButton.y - 126.0f * scale,
        firstButton.width + 140.0f * scale,
        (lastButton.y + lastButton.height - firstButton.y) + 178.0f * scale
    };
}
