...
static const BackpackEntryDef kBackpackEntries[BACKPACK_ENTRY_COUNT] = {
    ...
    {BACKPACK_ENTRY_RESOURCE, RESOURCE_FRUIT, LT("Plant Fruit", "植物果实"), LT("Soft, water-rich fruit from the marsh edge. Raw by itself, but useful as the fruit base of a processed survival ration.", "采自沼泽边缘的柔软果实，单独携带时只是原料，但很适合作为加工型生存口粮的果浆基底。"), Color{238, 127, 93, 255}, Color{142, 69, 49, 255}},
    {BACKPACK_ENTRY_RESOURCE, RESOURCE_SPECIAL_FUNGUS, LT("Special Fungus", "特殊菌株"), LT("An unstable fungal mass that reacts to toxins and leaking air paths. Dangerous to use raw, but ideal once it is stabilized into a sealed recovery mix.", "会对毒素和漏氧异常产生反应的不稳定菌株。直接处理并不安全，但很适合在密封复苏配方里被稳定下来。"), Color{164, 121, 220, 255}, Color{101, 70, 150, 255}},
    ...
    {BACKPACK_ENTRY_RESOURCE, RESOURCE_GLOW_MOSS, LT("Glow Moss", "发光苔"), LT("Cold bioluminescent moss that exhales a clean vapor when processed. It now works best as one ingredient in sealed recovery rations and support tools.", "这种冷色生物荧光苔在加工后会释放洁净气雾。它现在最适合拿去做密封复苏口粮和其他支援工具的组成部分。"), Color{119, 234, 170, 255}, Color{54, 144, 95, 255}},
    ...
    {BACKPACK_ENTRY_RESOURCE, RESOURCE_SHELL_FRUIT, LT("Shell Fruit", "壳果"), LT("A dense fruit sealed behind a brittle outer shell. Too coarse to use raw on the move now, but excellent once milled into a bench-made survival ration.", "外壳脆硬、果肉厚实的高密度果实。现在不会再直接生吃，而是更适合在工作台磨成果泥，做成应急生存口粮。"), Color{244, 188, 112, 255}, Color{151, 107, 55, 255}},
    ...
    {BACKPACK_ENTRY_RESOURCE, RESOURCE_CALM_MUSHROOM, LT("Calming Mushroom", "宁神蘑菇"), LT("A pale medicinal mushroom that settles breathing when properly processed. It is now treated as a recovery-ration ingredient, not a raw field snack.", "颜色苍白的药用蘑菇，在正确加工后能稳住呼吸节律。它现在被视为复苏口粮的材料，而不是能直接嚼下去的野外零食。"), Color{125, 178, 255, 255}, Color{71, 111, 188, 255}},
    ...
    {BACKPACK_ENTRY_RESOURCE, RESOURCE_RECOVERY_RATION, LT("Recovery Ration", "复苏口粮"), LT("A bench-sealed survival ration made from fruit pulp, medicinal fungus, calming mushroom, and glow-moss vapor. It is now the single quick-use consumable for restoring health, clearing poison, and recovering oxygen.", "在工作台完成密封的生存口粮，把果浆、药用菌株、宁神蘑菇和发光苔气雾压成一份综合应急包。它现在就是唯一的快捷消耗品，负责回血、解毒和补氧。"), Color{126, 212, 255, 255}, Color{57, 112, 172, 255}},
...
bool UIInventory_BackpackEntryCanUseDirectly(int itemIndex) {
    ...
    if (entry->kind == BACKPACK_ENTRY_RESOURCE) {
        return entry->itemId == RESOURCE_RECOVERY_RATION;
    }

    switch ((BackpackGearId)entry->itemId) {
        case BACKPACK_GEAR_GLOW_STICK:
        case BACKPACK_GEAR_ROPE:
            return true;
        default:
...
void UIInventory_GetBackpackEntryStatus(const Player *player, int itemIndex, char *buffer, size_t bufferSize) {
    ...
    if (!UIInventory_BackpackEntryIsOwned(player, itemIndex)) {
        std::snprintf(buffer, bufferSize, "%s: %s", Loc_PickLiteral("Status", "状态"), Loc_PickLiteral("Missing", "未拥有"));
        return;
    }

    switch ((BackpackGearId)entry->itemId) {
        case BACKPACK_GEAR_LASER_GUN:
            std::snprintf(buffer, bufferSize, "%s", Loc_PickLiteral("Equipped: press Space to fire", "已装备：按空格键开火"));
            break;
        case BACKPACK_GEAR_ROPE:
            std::snprintf(buffer, bufferSize, "%s", Loc_PickLiteral("Ready: face a rope barrier", "已就绪：面对绳索阻隔使用"));
            break;
        case BACKPACK_GEAR_PROTECTION_SUIT:
            std::snprintf(buffer, bufferSize, "%s", Loc_PickLiteral("Passive: sealed and active", "被动生效：已密封启用"));
            break;
        case BACKPACK_GEAR_FIELD_CAMP:
            std::snprintf(buffer, bufferSize, "%s", Loc_PickLiteral("Location use: rest at camp", "场景使用：在营地旁休整"));
            break;
        case BACKPACK_GEAR_SIGNAL_AMPLIFIER:
            std::snprintf(buffer, bufferSize, "%s", Loc_PickLiteral("Location use: tower route", "场景使用：用于塔楼路线"));
            break;
        case BACKPACK_GEAR_GLOW_STICK:
        default:
            std::snprintf(buffer, bufferSize, "%s", Loc_PickLiteral("Ready in pack", "已在背包中就绪"));
            break;
    }
}
...
RecipeVisualDef UIInventory_GetRecipeVisual(RecipeType recipe) {
    switch (recipe) {
        ...
        case RECIPE_RECOVERY_RATION:
            return RecipeVisualDef{Color{126, 212, 255, 255}, Color{57, 112, 172, 255}};
        case RECIPE_COUNT:
        default:
