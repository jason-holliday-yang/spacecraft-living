...
MapContentBounds GetNorthwestRuinsBounds(const GameMap *map) {
    const MapRegion *region = Map_GetRegionByName(map, "Northwest Ruins");

    if (region != nullptr && region->width > 0 && region->height > 0) {
        return MapContentBounds{
            region->gridX,
            region->gridY,
            region->gridX + region->width - 1,
            region->gridY + region->height - 1
        };
    }
    return MapContentBounds{
        BOSS_ARENA_X,
        BOSS_ARENA_Y,
        BOSS_ARENA_X + BOSS_ARENA_WIDTH - 1,
        BOSS_ARENA_Y + BOSS_ARENA_HEIGHT - 1
    };
}

bool IsNorthwestRuinsFightTile(const MapContentBounds &bounds, int gridX, int gridY) {
...
bool ShouldRevealNorthwestRuinsFightOnMap(const GameMap *map, const Player *player, const TaskSystem *tasks) {
    if (player != nullptr && Map_GetAreaAtMap(map, player->gridX, player->gridY) == MAP_AREA_BOSS_ARENA) {
        return true;
    }
    if (tasks == nullptr) {
        return false;
...
bool ShouldSkipMapTile(const MapContentBounds &northwestRuinsBounds,
                       int gridX,
                       int gridY,
                       bool revealNorthwestRuinsFight) {
    return !revealNorthwestRuinsFight && IsNorthwestRuinsFightTile(northwestRuinsBounds, gridX, gridY);
}

bool IsMapTileExplored(const MiniMap *minimap, const GameMap *map, int gridX, int gridY) {
    if (minimap == nullptr) {
        return true;
    }
    if (map == nullptr
...
bool IsContextInSameArea(const GameMap *map, int anchorX, int anchorY, int contextX, int contextY) {
    if (map == nullptr || !Map_IsWithinMapBounds(map, anchorX, anchorY) || !Map_IsWithinMapBounds(map, contextX, contextY)) {
        return false;
    }

    return Map_GetAreaAtMap(map, anchorX, anchorY) == Map_GetAreaAtMap(map, contextX, contextY);
}
...
bool IsContextInSameLocation(const GameMap *map, int anchorX, int anchorY, int contextX, int contextY) {
    ...
    anchorLocation = Map_GetLocationNameAtMap(map, anchorX, anchorY);
    contextLocation = Map_GetLocationNameAtMap(map, contextX, contextY);
...
bool ShouldShowMapAreaLabel(const MiniMap *minimap,
                            ...
                            const Player *player,
                            bool forceReveal) {
    if (forceReveal || IsMapTileExplored(minimap, map, anchorX, anchorY)) {
        return true;
    }
...
MapContentBounds GetMapContentBounds(const GameMap *map, bool revealNorthwestRuinsFight) {
    MapContentBounds bounds{map != nullptr ? map->width - 1 : 0, map != nullptr ? map->height - 1 : 0, 0, 0};
    const MapContentBounds northwestRuinsBounds = GetNorthwestRuinsBounds(map);
    bool found = false;

    if (map == nullptr) {
        return MapContentBounds{0, 0, 0, 0};
    }

    for (int y = 0; y < map->height; ++y) {
        for (int x = 0; x < map->width; ++x) {
            if (ShouldSkipMapTile(northwestRuinsBounds, x, y, revealNorthwestRuinsFight)) {
                continue;
            }
    ...
    if (!found) {
        return MapContentBounds{0, 0, map != nullptr ? map->width - 1 : 0, map != nullptr ? map->height - 1 : 0};
    }
...
void UI_DrawMapOverlay(const AssetBundle *assets,
    ...
    const bool revealNorthwestRuinsFight = ShouldRevealNorthwestRuinsFightOnMap(map, player, tasks);
    ...
    hasObjective = Tasks_GetObjectiveMarkerForMap(tasks, map, player, &objectiveX, &objectiveY);

    UIComponents_DrawScrim(screenWidth, screenHeight, 202);
    UIComponents_DrawFrame(panel, scale, UI_FRAME_STANDARD, true);
    UIComponents_DrawPanelHeader(assets,
                                 panel,
                                 Loc_PickLiteral("LOXI NAVIGATION ARRAY // SURVEY", "洛希导航阵列 // 勘测"),
                                 Loc_PickLiteral("AREA NAVIGATION", "区域导航"),
                                 Loc_PickLiteral("M / ESC closes", "M / ESC 关闭"),
                                 scale,
                                 theme->signalCyan);
    UIComponents_DrawCard(mapPanel, false, true, scale, theme->signalBlue);
    DrawRectangleGradientV((int)(mapPanel.x + 8.0f * scale),
                           (int)(mapPanel.y + 8.0f * scale),
                           (int)(mapPanel.width - 16.0f * scale),
                           (int)(mapPanel.height - 16.0f * scale),
                           Color{7, 24, 39, 120},
                           Color{4, 14, 25, 178});
    ...
    for (int y = contentBounds.minY; y <= contentBounds.maxY; ++y) {
        for (int x = contentBounds.minX; x <= contentBounds.maxX; ++x) {
            ...
            if (!IsMapTileExplored(minimap, map, x, y)) {
    ...
    if (hasObjective && objectiveX >= 0 && objectiveX < map->width && objectiveY >= 0 && objectiveY < map->height
        && (minimap == nullptr || IsMapTileExplored(minimap, map, objectiveX, objectiveY)
            || (revealNorthwestRuinsFight && IsNorthwestRuinsFightTile(northwestRuinsBounds, objectiveX, objectiveY)))) {
    ...
    if (tasks != nullptr) {
        for (int index = 0; index < tasks->logCount; ++index) {
            ...
            if (!IsMapTileExplored(minimap, map, log->gridX, log->gridY)) {
                continue;
            }
    ...
    if (revealNorthwestRuinsFight) {
        Rectangle northwestRuinsRect{
            TileScreenX(northwestRuinsBounds.minX),
            TileScreenY(northwestRuinsBounds.minY),
            (northwestRuinsBounds.maxX - northwestRuinsBounds.minX + 1) * cellSize,
            (northwestRuinsBounds.maxY - northwestRuinsBounds.minY + 1) * cellSize
        };
    ...
    UIComponents_DrawCard(legendCard, false, true, scale, theme->signalBlue);
    UIRuntime_DrawText(assets, Loc_PickLiteral("NAVIGATION KEY", "导航图例"), Vector2{legendCard.x + 16.0f * scale, legendCard.y + 14.0f * scale}, 18.0f * scale, WHITE);
    ...
    DrawLegendSwatch(assets, Rectangle{legendLeftX, legendRowY + 26.0f * scale, legendColumnWidth, 22.0f * scale}, theme->archiveGold, Loc_PickLiteral("Objective marker", "目标标记"), scale);
    ...
    DrawLegendSwatch(assets, Rectangle{legendLeftX, legendRowY + 78.0f * scale, legendColumnWidth, 22.0f * scale}, theme->signalCyan, Loc_PickLiteral("Supplemental archive", "补充档案"), scale);
