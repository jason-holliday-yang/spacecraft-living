...
static void DrawBossTelegraph(const Monster *monster, float elapsedSeconds) {
    ...
    arenaX = BOSS_ARENA_X;
    arenaY = BOSS_ARENA_Y;
    arenaWidth = BOSS_ARENA_WIDTH;
    arenaHeight = BOSS_ARENA_HEIGHT;
    arenaRegion = Map_GetRuntimeRegionByName("Northwest Ruins");
    if (arenaRegion != NULL && arenaRegion->width > 2 && arenaRegion->height > 2) {
        arenaX = arenaRegion->gridX;
        arenaY = arenaRegion->gridY;
        arenaWidth = arenaRegion->width;
        arenaHeight = arenaRegion->height;
    }
    ...
    switch (monster->currentAttack) {
        ...
        case BOSS_ATTACK_CHARGE: {
            ...
            if (IsBossLineTelegraphHorizontal(monster)) {
                const int lockedY = monster->targetY < arenaY + 1
                    ? arenaY + 1
                    : (monster->targetY > arenaY + arenaHeight - 2
                        ? arenaY + arenaHeight - 2
                        : monster->targetY);

                lineRect = Rectangle{
                    (float)((arenaX + 1) * TILE_SIZE),
                    (float)(lockedY * TILE_SIZE + TILE_SIZE / 2 - 5),
                    (float)((arenaWidth - 2) * TILE_SIZE),
                    10.0f + pulse * 4.0f
                };
            } else {
                const int lockedX = monster->targetX < arenaX + 1
                    ? arenaX + 1
                    : (monster->targetX > arenaX + arenaWidth - 2
                        ? arenaX + arenaWidth - 2
                        : monster->targetX);

                lineRect = Rectangle{
                    (float)(lockedX * TILE_SIZE + TILE_SIZE / 2 - 5),
                    (float)((arenaY + 1) * TILE_SIZE),
                    10.0f + pulse * 4.0f,
                    (float)((arenaHeight - 2) * TILE_SIZE)
                };
            }
...
void Tasks_DrawWorld(const TaskSystem *tasks, const AssetBundle *assets, float elapsedSeconds) {
    ...
    for (index = 0; index < tasks->nodeCount; index++) {
        ...
        if (Tasks_IsEntityOnActiveMap(tasks, node->mapId) && node->active) {
    ...
    for (index = 0; index < tasks->logCount; index++) {
        ...
        if (!Tasks_IsEntityOnActiveMap(tasks, log->mapId) || !log->active || log->collected) {
            continue;
        }
    ...
    for (index = 0; index < tasks->monsterCount; index++) {
        ...
        if (Tasks_IsEntityOnActiveMap(tasks, monster->mapId)
            && monster->active && tasks->stage >= monster->unlockStage) {
