...
static bool ResolvePropDrawRect(const GameMap *map, TileType tile, int gridX, int gridY, Rectangle *rect) {
    int originX;
    int originY;
    int width;
    int height;

    if (!Map_GetMultiTilePropBounds(map, tile, gridX, gridY, &originX, &originY, &width, &height)) {
        return true;
    }
...
void MapInternal_DrawPropCell(const GameMap *map,
    ...
    switch (propTile) {
        ...
        case TILE_TECH_TABLE:
            if (!ResolvePropDrawRect(map, propTile, gridX, gridY, &rect)) {
                break;
            }
        ...
        case TILE_BARRIER_RUINS:
            ...
            break;
        case TILE_ENTITY_BLOCKER:
            /* Collision-only Props tile. The matching Entity supplies visuals. */
            break;
        case TILE_VOID:
        default:
            break;
