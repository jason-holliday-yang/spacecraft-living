...
static bool TryBeginClickedPortal(Game *game) {
    Vector2 mouseWorld;
    const MapPortal *nearby;
    int index;

    if (!IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
        return false;
    }
    nearby = Map_FindPortalNear(&game->runtime.map, game->runtime.player.gridX, game->runtime.player.gridY);
    if (nearby == NULL) {
        return false;
    }
    mouseWorld = GetScreenToWorld2D(GetMousePosition(), game->runtime.camera);
    if (CheckCollisionPointRec(mouseWorld, (Rectangle){
            (float)(nearby->gridX * TILE_SIZE), (float)(nearby->gridY * TILE_SIZE),
            (float)(nearby->width * TILE_SIZE), (float)(nearby->height * TILE_SIZE)})) {
        return Game_BeginPortalTransition(game, nearby);
    }
    for (index = 0; index < game->runtime.map.entityCount; index++) {
        const MapEntity *entity = &game->runtime.map.entities[index];
        if (strcmp(entity->portalId, nearby->portalId) == 0
            && CheckCollisionPointRec(mouseWorld, (Rectangle){
                (float)(entity->gridX * TILE_SIZE), (float)(entity->gridY * TILE_SIZE),
                (float)(entity->width * TILE_SIZE), (float)(entity->height * TILE_SIZE)})) {
            return Game_BeginPortalTransition(game, nearby);
        }
    }
    return false;
...
static void GamePlay_StartLaserEffect(Game *game, const Monster *target, bool hit) {
    if (game == NULL || !game->runtime.player.hasLaserGun) {
        return;
    }

    game->runtime.laserEffectTimer = 0.18f;
    game->runtime.laserEffectHit = hit && target != NULL;
    game->runtime.laserEffectStart = (Vector2){
        game->runtime.player.renderPos.x,
        game->runtime.player.renderPos.y - (float)TILE_SIZE * 0.28f
    };
    game->runtime.laserEffectEnd = target != NULL ? GetMonsterCenterWorld(target) : GetLaserMissEndWorld(&game->runtime.player);
}
...
static bool IsCrewQuartersInteraction(const Game *game, TaskInteractionTarget target) {
    ...
    if (game == NULL || target != TASK_INTERACTION_NONE || Map_GetAreaAtMap(&game->runtime.map, game->runtime.player.gridX, game->runtime.player.gridY) != MAP_AREA_BASE) {
        return false;
    }

    roomName = Map_GetRoomNameAtMap(&game->runtime.map, game->runtime.player.gridX, game->runtime.player.gridY);
...
static bool IsNearLoxiTerminal(const Game *game) {
    ...
    terminalX = LOXI_TERMINAL_X;
    terminalY = LOXI_TERMINAL_Y;
    Map_GetAnchorPosition(&game->runtime.map, MAP_ANCHOR_LOXI_TERMINAL, &terminalX, &terminalY);
    return game->runtime.player.gridX >= terminalX - 2
        && game->runtime.player.gridX <= terminalX + STATION_FOOTPRINT_WIDTH + 1
        && game->runtime.player.gridY >= terminalY - 2
        && game->runtime.player.gridY <= terminalY + STATION_FOOTPRINT_HEIGHT + 1;
}
...
static bool ShouldOpenSettlementConfirm(const Game *game) {
    ...
    return game != NULL
        ...
        && game->runtime.tasks.endingArchiveReviewed
...
static void HandleMovement(Game *game, int deltaX, int deltaY) {
    ...
    if (deltaX != 0 || deltaY != 0) {
        game->runtime.player.facingX = deltaX;
        game->runtime.player.facingY = deltaY;
    }

    targetX = game->runtime.player.gridX + deltaX;
    targetY = game->runtime.player.gridY + deltaY;

    if (Tasks_IsBlockingActorTile(&game->runtime.tasks, targetX, targetY)) {
        Game_PostMessage(game, Loc_PickLiteral("A hostile creature is blocking the path.", "有敌对生物挡住了去路。"), 2.4f);
        game->runtime.player.moveTimer = BLOCKED_MOVE_COOLDOWN;
        return;
    }

    startPos = game->runtime.player.renderPos;
    moved = Player_Move(&game->runtime.player, &game->runtime.map, deltaX, deltaY);
    if (moved) {
        moveDuration = Player_GetMoveCooldown(&game->runtime.player);
        Player_StartMoveAnimation(&game->runtime.player, startPos, moveDuration);
        game->runtime.player.moveTimer = moveDuration;
        game->input.inputBufferTimer = 0.0f;
        game->input.bufferedMoveX = 0;
        game->input.bufferedMoveY = 0;
        Audio_PlayCue(&game->services.audio,
                      GetFootstepCueForTile(Map_GetGroundTileAt(&game->runtime.map, game->runtime.player.gridX, game->runtime.player.gridY),
                                            Map_GetAreaAtMap(&game->runtime.map, game->runtime.player.gridX, game->runtime.player.gridY)));
        return;
    }
...
void GamePlay_UpdateMovement(Game *game) {
    ...
    if (hasPressedInput) {
        ...
        if (game->runtime.player.moveTimer > 0.0f && queuedBufferTime < game->runtime.player.moveTimer + kBufferedMoveGrace) {
            queuedBufferTime = game->runtime.player.moveTimer + kBufferedMoveGrace;
        }
        game->input.bufferedMoveX = pressedX;
        game->input.bufferedMoveY = pressedY;
        game->input.inputBufferTimer = queuedBufferTime;
        game->input.heldMoveX = pressedX;
        game->input.heldMoveY = pressedY;
        game->input.holdRepeatTimer = HOLD_REPEAT_INITIAL_DELAY;
    }
    ...
    } else if (!SameDirection(game->input.heldMoveX, game->input.heldMoveY, heldX, heldY)) {
        game->input.heldMoveX = heldX;
        game->input.heldMoveY = heldY;
        game->input.holdRepeatTimer = HOLD_REPEAT_INITIAL_DELAY;
    } else if (!hasPressedInput && game->input.holdRepeatTimer <= 0.0f) {
        game->input.bufferedMoveX = heldX;
        game->input.bufferedMoveY = heldY;
        game->input.inputBufferTimer = INPUT_BUFFER_TIME;
        game->input.holdRepeatTimer = HOLD_REPEAT_INTERVAL;
    }

    if (game->runtime.player.moveTimer > 0.0f) {
        if (hasHeldInput
            && (heldX != 0 || heldY != 0)
            && (hasPressedInput || game->input.holdRepeatTimer <= kBufferedMoveGrace)) {
            queuedBufferTime = game->runtime.player.moveTimer + kBufferedMoveGrace;
            game->input.bufferedMoveX = heldX;
            game->input.bufferedMoveY = heldY;
            if (game->input.inputBufferTimer < queuedBufferTime) {
                game->input.inputBufferTimer = queuedBufferTime;
            }
        }
        return;
    ...
    if (game->input.inputBufferTimer > 0.0f && (game->input.bufferedMoveX != 0 || game->input.bufferedMoveY != 0)) {
        HandleMovement(game, game->input.bufferedMoveX, game->input.bufferedMoveY);
    }
}
...
bool GamePlay_HandleImmediateInput(Game *game, char *actionMessage, size_t messageSize) {
    ...
    if (TryBeginClickedPortal(game)) {
        return true;
    }
    if (IsKeyPressed(KEY_F)) {
        const MapPortal *portal = Map_FindPortalNear(&game->runtime.map, game->runtime.player.gridX, game->runtime.player.gridY);
        if (portal != NULL && Game_BeginPortalTransition(game, portal)) {
            return true;
        }
        ...
        shouldPrioritizePickup = TasksRuntime_HasNearbyPickupPriority(&game->runtime.tasks, &game->runtime.player);
        preferredTarget = TasksRuntime_GetPreferredInteractionTarget(&game->runtime.map, &game->runtime.player);
        ...
        if (preferredTarget != TASK_INTERACTION_LOXI_TERMINAL) {
            nearbyNode = TasksRuntime_FindNearbyNode(&game->runtime.tasks, &game->runtime.player);
            if (nearbyNode == NULL) {
                nearbyLog = TasksRuntime_FindNearbyLog(&game->runtime.tasks, &game->runtime.player);
            }
        }
        ...
        endingBeforeInteraction = game->runtime.tasks.ending;
        if (!shouldPrioritizePickup && preferredTarget == TASK_INTERACTION_WORKBENCH) {
            game->ui.craftOpen = true;
            game->ui.selectedCraftIndex = 0;
            Audio_PlayCue(&game->services.audio, AUDIO_CUE_OPEN);
            return true;
        }

        if (Tasks_HandleInteraction(&game->runtime.tasks, &game->runtime.map, &game->runtime.player, actionMessage, messageSize)) {
            if (actionMessage[0] != '\0' && game->runtime.tasks.ending == endingBeforeInteraction) {
                Audio_PlayCue(&game->services.audio, SelectInteractionCue(actionMessage, preferredTarget, nearbyNode, nearbyLog));
            }
            if (actionMessage[0] != '\0') {
                Game_PostMessage(game, actionMessage, 4.0f);
            }
            if (shouldStartSleepTransition && game->runtime.tasks.ending == endingBeforeInteraction) {
    ...
    if (IsKeyPressed(KEY_SPACE)) {
        ...
        attackTarget = game->runtime.player.hasLaserGun ? TasksRuntime_FindAttackTarget(&game->runtime.tasks, &game->runtime.player) : NULL;
        if (Tasks_HandleAttack(&game->runtime.tasks, &game->runtime.map, &game->runtime.player, actionMessage, messageSize)) {
            GamePlay_StartLaserEffect(game, attackTarget, attackTarget != NULL);
            Audio_PlayCue(&game->services.audio, game->runtime.player.hasLaserGun ? AUDIO_CUE_LASER : AUDIO_CUE_MELEE);
    ...
    if (IsKeyPressed(KEY_X)) {
        if (Player_UseQuickConsumable(&game->runtime.player, actionMessage, (int)messageSize)) {
            Game_PostMessage(game, actionMessage, 2.8f);
        } else {
