...
static void GamePlay_UpdateGameplayTimers(Game *game, float deltaTime) {
    ...
    if (game->runtime.player.moveTimer > 0.0f) {
        game->runtime.player.moveTimer -= deltaTime;
        if (game->runtime.player.moveTimer < 0.0f) {
            game->runtime.player.moveTimer = 0.0f;
        }
    }

    Player_UpdateAnimation(&game->runtime.player, deltaTime);

    if (game->input.inputBufferTimer > 0.0f) {
        game->input.inputBufferTimer -= deltaTime;
        if (game->input.inputBufferTimer <= 0.0f) {
    ...
    if (game->input.holdRepeatTimer > 0.0f) {
        game->input.holdRepeatTimer -= deltaTime;
        if (game->input.holdRepeatTimer < 0.0f) {
            game->input.holdRepeatTimer = 0.0f;
        }
    }

    if (game->runtime.laserEffectTimer > 0.0f) {
        game->runtime.laserEffectTimer -= deltaTime;
        if (game->runtime.laserEffectTimer < 0.0f) {
...
static void GamePlay_CaptureFeedbackSnapshot(const Game *game, GameplayFeedbackSnapshot *snapshot) {
    ...
    snapshot->lowOxygen = Player_HasStatus(&game->runtime.player, PLAYER_STATUS_LOW_OXYGEN);
    snapshot->oxygenLeak = Player_HasStatus(&game->runtime.player, PLAYER_STATUS_OXYGEN_LEAK);
    snapshot->suffocating = Player_HasStatus(&game->runtime.player, PLAYER_STATUS_SUFFOCATING);
    snapshot->bossTelegraph = HasActiveBossTelegraph(&game->runtime.tasks);
    snapshot->bossPhase = GetActiveBossPhase(&game->runtime.tasks);
...
static void GamePlay_UpdateGameplaySimulation(Game *game, float deltaTime, const GameplayFeedbackSnapshot *before) {
    Tasks_Update(&game->runtime.tasks, &game->runtime.map, &game->runtime.player, deltaTime);
...
static void GamePlay_UpdateGameplayFeedback(Game *game, const GameplayFeedbackSnapshot *before) {
    ...
    const bool healthDropped = game->runtime.player.health + 0.05f < before->health;
    const bool poisonIncreased = game->runtime.player.poison > before->poison + 0.5f;
    const bool lowOxygenStarted = !before->lowOxygen && Player_HasStatus(&game->runtime.player, PLAYER_STATUS_LOW_OXYGEN);
    const bool oxygenLeakStarted = !before->oxygenLeak && Player_HasStatus(&game->runtime.player, PLAYER_STATUS_OXYGEN_LEAK);
    const bool suffocatingNow = Player_HasStatus(&game->runtime.player, PLAYER_STATUS_SUFFOCATING);
    ...
    if ((healthDropped || poisonIncreased || lowOxygenStarted || oxygenLeakStarted)
        ...
        Audio_PlayCue(&game->services.audio, AUDIO_CUE_HURT);
        game->runtime.hurtSoundCooldown = suffocationDamageTriggered
            ? kSuffocationHurtCueCooldown
            : kGeneralHurtCueCooldown;
    }

    if (BossStartedTelegraphing(&game->runtime.tasks, before->bossTelegraph) && game->runtime.monsterCueCooldown <= 0.0f) {
        Audio_PlayCue(&game->services.audio, AUDIO_CUE_MONSTER);
        game->runtime.monsterCueCooldown = 1.25f;
    }

    const int bossPhaseNow = GetActiveBossPhase(&game->runtime.tasks);
    if (before->bossPhase == 1 && bossPhaseNow == 2) {
        ...
        Audio_PlayCue(&game->services.audio, AUDIO_CUE_WARNING);
    } else if (before->bossPhase >= 1 && bossPhaseNow == 0 && game->runtime.tasks.bossDefeated) {
...
static bool GamePlay_UpdateGameplayFlow(Game *game,
    ...
    if (game->runtime.tasks.ending == ENDING_NONE && game->runtime.player.health <= 0.0f) {
        Game_HandlePlayerDeath(game);
        return true;
    }

    if (!game->flow.storySceneOpen && game->runtime.tasks.ending != ENDING_NONE) {
...
static void GamePlay_UpdateCamera(Game *game, float deltaTime) {
    ...
    const Vector2 desiredTarget = game->runtime.player.renderPos;
    const float deltaX = desiredTarget.x - game->runtime.camera.target.x;
    const float deltaY = desiredTarget.y - game->runtime.camera.target.y;
    const float distanceSquared = deltaX * deltaX + deltaY * deltaY;

    if (distanceSquared >= kCameraSnapDistance * kCameraSnapDistance) {
        game->runtime.camera.target = desiredTarget;
    } else {
        const float followT = ClampUnit(deltaTime * kCameraCatchUpSpeed);
        game->runtime.camera.target = LerpVector2(game->runtime.camera.target, desiredTarget, followT);
    }
