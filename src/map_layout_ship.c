#include "map_internal.h"

static const int kBunkOrigins[][2] = {
    {45, 44}
};

static const int kStorageLockerOrigins[][2] = {
    {42, 51}
};

static const int kTechTableOrigins[][2] = {
    {45, 58}
};

static bool IsRectBounds(int gridX, int gridY, int x, int y, int width, int height) {
    return gridX >= x && gridX < x + width && gridY >= y && gridY < y + height;
}

static bool ResolveSquarePropBounds(int gridX,
                                    int gridY,
                                    int candidateOriginX,
                                    int candidateOriginY,
                                    int size,
                                    int *originX,
                                    int *originY,
                                    int *width,
                                    int *height) {
    if (gridX < candidateOriginX || gridX >= candidateOriginX + size
        || gridY < candidateOriginY || gridY >= candidateOriginY + size) {
        return false;
    }

    *originX = candidateOriginX;
    *originY = candidateOriginY;
    *width = size;
    *height = size;
    return true;
}

static bool ResolveSquarePropBoundsFromOrigins(int gridX,
                                               int gridY,
                                               const int (*origins)[2],
                                               int originCount,
                                               int size,
                                               int *originX,
                                               int *originY,
                                               int *width,
                                               int *height) {
    int index;

    for (index = 0; index < originCount; index++) {
        if (ResolveSquarePropBounds(gridX,
                                    gridY,
                                    origins[index][0],
                                    origins[index][1],
                                    size,
                                    originX,
                                    originY,
                                    width,
                                    height)) {
            return true;
        }
    }

    return false;
}

static void FillPropRectsFromOrigins(GameMap *map,
                                     const int (*origins)[2],
                                     int originCount,
                                     int size,
                                     TileType tile) {
    int index;

    for (index = 0; index < originCount; index++) {
        MapInternal_FillPropRect(map, origins[index][0], origins[index][1], size, size, tile);
    }
}

static void FillVerticalConnector(GameMap *map, int gridX, int startY, int endY) {
    int topY;
    int height;

    if (startY > endY) {
        topY = endY;
        height = startY - endY + 1;
    } else {
        topY = startY;
        height = endY - startY + 1;
    }

    MapInternal_FillGroundRect(map, gridX, topY, SHIP_PASSAGE_WIDTH, height, TILE_BASE_FLOOR);
}

bool MapInternal_IsShipRoomBounds(int gridX, int gridY) {
    return IsRectBounds(gridX, gridY, SHIP_CORRIDOR_X, SHIP_CORRIDOR_Y, SHIP_CORRIDOR_WIDTH, SHIP_CORRIDOR_HEIGHT)
        || IsRectBounds(gridX, gridY, SHIP_CARGO_HOLD_X, SHIP_CARGO_HOLD_Y, SHIP_CARGO_HOLD_WIDTH, SHIP_CARGO_HOLD_HEIGHT)
        || IsRectBounds(gridX, gridY, SHIP_CREW_QUARTERS_X, SHIP_CREW_QUARTERS_Y, SHIP_CREW_QUARTERS_WIDTH, SHIP_CREW_QUARTERS_HEIGHT)
        || IsRectBounds(gridX, gridY, SHIP_DIAGNOSTICS_X, SHIP_DIAGNOSTICS_Y, SHIP_DIAGNOSTICS_WIDTH, SHIP_DIAGNOSTICS_HEIGHT)
        || IsRectBounds(gridX, gridY, SHIP_TERMINAL_BAY_X, SHIP_TERMINAL_BAY_Y, SHIP_TERMINAL_BAY_WIDTH, SHIP_TERMINAL_BAY_HEIGHT)
        || IsRectBounds(gridX, gridY, SHIP_LIFE_SUPPORT_X, SHIP_LIFE_SUPPORT_Y, SHIP_LIFE_SUPPORT_WIDTH, SHIP_LIFE_SUPPORT_HEIGHT)
        || IsRectBounds(gridX, gridY, SHIP_WORKSHOP_X, SHIP_WORKSHOP_Y, SHIP_WORKSHOP_WIDTH, SHIP_WORKSHOP_HEIGHT)
        || IsRectBounds(gridX, gridY, SHIP_POWER_BAY_X, SHIP_POWER_BAY_Y, SHIP_POWER_BAY_WIDTH, SHIP_POWER_BAY_HEIGHT)
        || IsRectBounds(gridX, gridY, SHIP_AIRLOCK_LINK_X, SHIP_AIRLOCK_LINK_Y, SHIP_AIRLOCK_LINK_WIDTH, SHIP_AIRLOCK_LINK_HEIGHT);
}

bool MapInternal_GetMultiTilePropBounds(TileType tile,
                                        int gridX,
                                        int gridY,
                                        int *originX,
                                        int *originY,
                                        int *width,
                                        int *height) {
    switch (tile) {
        case TILE_TECH_TABLE:
            return ResolveSquarePropBoundsFromOrigins(gridX,
                                                      gridY,
                                                      kTechTableOrigins,
                                                      (int)(sizeof(kTechTableOrigins) / sizeof(kTechTableOrigins[0])),
                                                      SHIP_FURNITURE_FOOTPRINT_SIZE,
                                                      originX,
                                                      originY,
                                                      width,
                                                      height);
        case TILE_STORAGE_LOCKER:
            return ResolveSquarePropBoundsFromOrigins(gridX,
                                                      gridY,
                                                      kStorageLockerOrigins,
                                                      (int)(sizeof(kStorageLockerOrigins) / sizeof(kStorageLockerOrigins[0])),
                                                      SHIP_FURNITURE_FOOTPRINT_SIZE,
                                                      originX,
                                                      originY,
                                                      width,
                                                      height);
        case TILE_BUNK:
            return ResolveSquarePropBoundsFromOrigins(gridX,
                                                      gridY,
                                                      kBunkOrigins,
                                                      (int)(sizeof(kBunkOrigins) / sizeof(kBunkOrigins[0])),
                                                      SHIP_FURNITURE_FOOTPRINT_SIZE,
                                                      originX,
                                                      originY,
                                                      width,
                                                      height);
        case TILE_OXYGEN_CONSOLE:
            *originX = OXYGEN_CONSOLE_X;
            *originY = OXYGEN_CONSOLE_Y;
            *width = STATION_FOOTPRINT_WIDTH;
            *height = STATION_FOOTPRINT_HEIGHT;
            break;
        case TILE_LOXI_TERMINAL:
            *originX = LOXI_TERMINAL_X;
            *originY = LOXI_TERMINAL_Y;
            *width = STATION_FOOTPRINT_WIDTH;
            *height = STATION_FOOTPRINT_HEIGHT;
            break;
        case TILE_WORKBENCH:
            *originX = WORKBENCH_X;
            *originY = WORKBENCH_Y;
            *width = STATION_FOOTPRINT_WIDTH;
            *height = STATION_FOOTPRINT_HEIGHT;
            break;
        case TILE_AIRLOCK_CONSOLE:
            *originX = AIRLOCK_CONSOLE_X;
            *originY = AIRLOCK_CONSOLE_Y;
            *width = STATION_FOOTPRINT_WIDTH;
            *height = STATION_FOOTPRINT_HEIGHT;
            break;
        case TILE_ENERGY_CONSOLE:
            *originX = ENERGY_CONSOLE_X;
            *originY = ENERGY_CONSOLE_Y;
            *width = STATION_FOOTPRINT_WIDTH;
            *height = STATION_FOOTPRINT_HEIGHT;
            break;
        case TILE_COMM_RELAY:
            *originX = COMM_RELAY_X;
            *originY = COMM_RELAY_Y;
            *width = WORLD_INTERACTIVE_FOOTPRINT_SIZE;
            *height = WORLD_INTERACTIVE_FOOTPRINT_SIZE;
            break;
        case TILE_CRASH_CLUE:
            *originX = CRASH_CLUE_X;
            *originY = CRASH_CLUE_Y;
            *width = CRASH_CLUE_FOOTPRINT_SIZE;
            *height = CRASH_CLUE_FOOTPRINT_SIZE;
            break;
        case TILE_SIGNAL_TOWER:
            *originX = SIGNAL_TOWER_X;
            *originY = SIGNAL_TOWER_Y;
            *width = WORLD_INTERACTIVE_FOOTPRINT_SIZE;
            *height = WORLD_INTERACTIVE_FOOTPRINT_SIZE;
            break;
        case TILE_MONOLITH:
            if (ResolveSquarePropBounds(gridX,
                                        gridY,
                                        MONOLITH_A_X,
                                        MONOLITH_A_Y,
                                        WORLD_INTERACTIVE_FOOTPRINT_SIZE,
                                        originX,
                                        originY,
                                        width,
                                        height)
                || ResolveSquarePropBounds(gridX,
                                           gridY,
                                           MONOLITH_B_X,
                                           MONOLITH_B_Y,
                                           WORLD_INTERACTIVE_FOOTPRINT_SIZE,
                                           originX,
                                           originY,
                                           width,
                                           height)
                || ResolveSquarePropBounds(gridX,
                                           gridY,
                                           MONOLITH_C_X,
                                           MONOLITH_C_Y,
                                           WORLD_INTERACTIVE_FOOTPRINT_SIZE,
                                           originX,
                                           originY,
                                           width,
                                           height)) {
                return true;
            }
            return false;
        default:
            return false;
    }

    return gridX >= *originX && gridX < *originX + *width
        && gridY >= *originY && gridY < *originY + *height;
}

void MapInternal_SeedShipLayout(GameMap *map) {
    MapInternal_FillGroundRect(map, SHIP_CORRIDOR_X, SHIP_CORRIDOR_Y, SHIP_CORRIDOR_WIDTH, SHIP_CORRIDOR_HEIGHT, TILE_BASE_FLOOR);
    MapInternal_FillGroundRect(map, SHIP_CARGO_HOLD_X, SHIP_CARGO_HOLD_Y, SHIP_CARGO_HOLD_WIDTH, SHIP_CARGO_HOLD_HEIGHT, TILE_BASE_FLOOR);
    MapInternal_FillGroundRect(map, SHIP_CREW_QUARTERS_X, SHIP_CREW_QUARTERS_Y, SHIP_CREW_QUARTERS_WIDTH, SHIP_CREW_QUARTERS_HEIGHT, TILE_BASE_FLOOR);
    MapInternal_FillGroundRect(map, SHIP_DIAGNOSTICS_X, SHIP_DIAGNOSTICS_Y, SHIP_DIAGNOSTICS_WIDTH, SHIP_DIAGNOSTICS_HEIGHT, TILE_BASE_FLOOR);
    MapInternal_FillGroundRect(map, SHIP_TERMINAL_BAY_X, SHIP_TERMINAL_BAY_Y, SHIP_TERMINAL_BAY_WIDTH, SHIP_TERMINAL_BAY_HEIGHT, TILE_BASE_FLOOR);
    MapInternal_FillGroundRect(map, SHIP_LIFE_SUPPORT_X, SHIP_LIFE_SUPPORT_Y, SHIP_LIFE_SUPPORT_WIDTH, SHIP_LIFE_SUPPORT_HEIGHT, TILE_BASE_FLOOR);
    MapInternal_FillGroundRect(map, SHIP_WORKSHOP_X, SHIP_WORKSHOP_Y, SHIP_WORKSHOP_WIDTH, SHIP_WORKSHOP_HEIGHT, TILE_BASE_FLOOR);
    MapInternal_FillGroundRect(map, SHIP_POWER_BAY_X, SHIP_POWER_BAY_Y, SHIP_POWER_BAY_WIDTH, SHIP_POWER_BAY_HEIGHT, TILE_BASE_FLOOR);
    MapInternal_FillGroundRect(map, SHIP_AIRLOCK_LINK_X, SHIP_AIRLOCK_LINK_Y, SHIP_AIRLOCK_LINK_WIDTH, SHIP_AIRLOCK_LINK_HEIGHT, TILE_BASE_FLOOR);
    MapInternal_FillGroundRect(map,
                               AIRLOCK_CONSOLE_X - 1,
                               AIRLOCK_CONSOLE_Y,
                               STATION_FOOTPRINT_WIDTH + 1,
                               STATION_FOOTPRINT_HEIGHT,
                               TILE_BASE_FLOOR);
    FillVerticalConnector(map, SHIP_CREW_QUARTERS_X + 1, SHIP_CREW_QUARTERS_Y + SHIP_CREW_QUARTERS_HEIGHT, SHIP_CORRIDOR_Y - 1);
    FillVerticalConnector(map, SHIP_TERMINAL_BAY_X + 1, SHIP_TERMINAL_BAY_Y + SHIP_TERMINAL_BAY_HEIGHT, SHIP_CORRIDOR_Y - 1);
    FillVerticalConnector(map, SHIP_WORKSHOP_X + 1, SHIP_WORKSHOP_Y + SHIP_WORKSHOP_HEIGHT, SHIP_CORRIDOR_Y - 1);
    FillVerticalConnector(map, SHIP_DIAGNOSTICS_X + 1, SHIP_CORRIDOR_Y + SHIP_CORRIDOR_HEIGHT, SHIP_DIAGNOSTICS_Y - 1);
    FillVerticalConnector(map, SHIP_LIFE_SUPPORT_X + 1, SHIP_CORRIDOR_Y + SHIP_CORRIDOR_HEIGHT, SHIP_LIFE_SUPPORT_Y - 1);
    FillVerticalConnector(map, SHIP_POWER_BAY_X + 1, SHIP_CORRIDOR_Y + SHIP_CORRIDOR_HEIGHT, SHIP_POWER_BAY_Y - 1);
    MapInternal_FillPropRect(map, OXYGEN_CONSOLE_X, OXYGEN_CONSOLE_Y, STATION_FOOTPRINT_WIDTH, STATION_FOOTPRINT_HEIGHT, TILE_OXYGEN_CONSOLE);
    MapInternal_FillPropRect(map, LOXI_TERMINAL_X, LOXI_TERMINAL_Y, STATION_FOOTPRINT_WIDTH, STATION_FOOTPRINT_HEIGHT, TILE_LOXI_TERMINAL);
    MapInternal_FillPropRect(map, WORKBENCH_X, WORKBENCH_Y, STATION_FOOTPRINT_WIDTH, STATION_FOOTPRINT_HEIGHT, TILE_WORKBENCH);
    MapInternal_FillPropRect(map, AIRLOCK_CONSOLE_X, AIRLOCK_CONSOLE_Y, STATION_FOOTPRINT_WIDTH, STATION_FOOTPRINT_HEIGHT, TILE_AIRLOCK_CONSOLE);
    MapInternal_FillPropRect(map, ENERGY_CONSOLE_X, ENERGY_CONSOLE_Y, STATION_FOOTPRINT_WIDTH, STATION_FOOTPRINT_HEIGHT, TILE_ENERGY_CONSOLE);

    MapInternal_FillGroundRect(map, AIRLOCK_DOOR_X, AIRLOCK_DOOR_TOP_Y, 1, AIRLOCK_DOOR_HEIGHT, TILE_BASE_FLOOR);
    MapInternal_FillPropRect(map, AIRLOCK_DOOR_X, AIRLOCK_DOOR_TOP_Y, 1, AIRLOCK_DOOR_HEIGHT, TILE_AIRLOCK_DOOR);

    FillPropRectsFromOrigins(map,
                             kBunkOrigins,
                             (int)(sizeof(kBunkOrigins) / sizeof(kBunkOrigins[0])),
                             SHIP_FURNITURE_FOOTPRINT_SIZE,
                             TILE_BUNK);
    FillPropRectsFromOrigins(map,
                             kStorageLockerOrigins,
                             (int)(sizeof(kStorageLockerOrigins) / sizeof(kStorageLockerOrigins[0])),
                             SHIP_FURNITURE_FOOTPRINT_SIZE,
                             TILE_STORAGE_LOCKER);
    FillPropRectsFromOrigins(map,
                             kTechTableOrigins,
                             (int)(sizeof(kTechTableOrigins) / sizeof(kTechTableOrigins[0])),
                             SHIP_FURNITURE_FOOTPRINT_SIZE,
                             TILE_TECH_TABLE);
}
