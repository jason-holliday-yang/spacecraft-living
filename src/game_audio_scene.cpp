...
AudioMusicStage Game_SelectMusicStage(const Game *game) {
    ...
    if (game->state == GAME_STATE_OPENING) {
        return GetStageMusicStage(game->tasks.stage);
    }

    if (game->tasks.ending != ENDING_NONE || game->state == GAME_STATE_ENDING) {
    ...
    switch (game->tasks.selectedEndingRoute) {
        case ENDING_HEROIC:
...
AudioScene Game_SelectAudioScene(const Game *game) {
    ...
    if (game->state == GAME_STATE_INTRO || game->state == GAME_STATE_OPENING) {
