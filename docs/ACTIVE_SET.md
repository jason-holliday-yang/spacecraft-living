# SpaceCraft Living 开发必读集（Active Set）

本文件只保留“下一步开发直接需要”的文档。

使用原则：

- 开工前先读“当前阶段必读”清单，并优先核对当前运行时口径。
- 具体改代码时按“任务扩展阅读”补看。
- 涉及“当前已经实装到哪一步”的事实时，以 `CURRENT_STATUS.md` 和 `STAGE_FLOW_CODE_VERIFIED.md` 为准。
- 如与历史文档冲突，以本文件列出的活动文档为准。

## 当前阶段必读（先读）

1. [`design/GAME_DESIGN.md`](/Users/jason/Documents/SpaceCraftLivng/docs/design/GAME_DESIGN.md)
2. [`design/CURRENT_STATUS.md`](/Users/jason/Documents/SpaceCraftLivng/docs/design/CURRENT_STATUS.md)
3. [`design/TERMINOLOGY.md`](/Users/jason/Documents/SpaceCraftLivng/docs/design/TERMINOLOGY.md)
4. [`design/STAGE_FLOW_CODE_VERIFIED.md`](/Users/jason/Documents/SpaceCraftLivng/docs/design/STAGE_FLOW_CODE_VERIFIED.md)
5. [`plans/NEXT_STEPS.md`](/Users/jason/Documents/SpaceCraftLivng/docs/plans/NEXT_STEPS.md)
6. [`plans/MAP_STORY_ALIGNMENT_PLAN.md`](/Users/jason/Documents/SpaceCraftLivng/docs/plans/MAP_STORY_ALIGNMENT_PLAN.md)
7. [`plans/BALANCE_TUNING_PLAN.md`](/Users/jason/Documents/SpaceCraftLivng/docs/plans/BALANCE_TUNING_PLAN.md)
8. [`engineering/ARCHITECTURE.md`](/Users/jason/Documents/SpaceCraftLivng/docs/engineering/ARCHITECTURE.md)
9. [`design/PLAY_GUIDE.md`](/Users/jason/Documents/SpaceCraftLivng/docs/design/PLAY_GUIDE.md)
10. [`plans/STAGE_MUSIC_PLAYBACK_PLAN.md`](/Users/jason/Documents/SpaceCraftLivng/docs/plans/STAGE_MUSIC_PLAYBACK_PLAN.md)

## 任务扩展阅读（按需）

- 西线 / 南线地图职责：
  [`design/EXPANSION_MAP_DESIGN.md`](/Users/jason/Documents/SpaceCraftLivng/docs/design/EXPANSION_MAP_DESIGN.md)
- 西线 / 南线任务结构：
  [`design/EXPANSION_STORY_TASKS.md`](/Users/jason/Documents/SpaceCraftLivng/docs/design/EXPANSION_STORY_TASKS.md)
- 西线 / 南线完整剧情：
  [`design/WEST_SOUTH_FULL_STORY.md`](/Users/jason/Documents/SpaceCraftLivng/docs/design/WEST_SOUTH_FULL_STORY.md)
- 长篇剧情母稿（写作层参考）：
  [`design/LONGFORM_STORY_DRAFT.md`](/Users/jason/Documents/SpaceCraftLivng/docs/design/LONGFORM_STORY_DRAFT.md)
- 剧情文档修正清单：
  [`plans/STORY_DOCUMENT_REPAIR_CHECKLIST.md`](/Users/jason/Documents/SpaceCraftLivng/docs/plans/STORY_DOCUMENT_REPAIR_CHECKLIST.md)
- 日志详细描述与资源 Brief：
  [`design/LOG_ARCHIVE_DETAIL_BRIEFS.md`](/Users/jason/Documents/SpaceCraftLivng/docs/design/LOG_ARCHIVE_DETAIL_BRIEFS.md)
- 主线剧情详细描述与资源 Brief：
  [`design/MAINLINE_STORY_DETAIL_BRIEFS.md`](/Users/jason/Documents/SpaceCraftLivng/docs/design/MAINLINE_STORY_DETAIL_BRIEFS.md)
- 西线 / 南线资源缺口：
  [`design/WEST_SOUTH_RESOURCE_GAP.md`](/Users/jason/Documents/SpaceCraftLivng/docs/design/WEST_SOUTH_RESOURCE_GAP.md)
- 状态系统与生存规则映射：
  [`design/STATUS_SYSTEM_MAPPING.md`](/Users/jason/Documents/SpaceCraftLivng/docs/design/STATUS_SYSTEM_MAPPING.md)
- 阶段化 BGM 播放改造：
  [`plans/STAGE_MUSIC_PLAYBACK_PLAN.md`](/Users/jason/Documents/SpaceCraftLivng/docs/plans/STAGE_MUSIC_PLAYBACK_PLAN.md)
- 对外试玩与流程核对：
  [`design/PLAY_GUIDE.md`](/Users/jason/Documents/SpaceCraftLivng/docs/design/PLAY_GUIDE.md)

## 资源协作文档（按需）

- [`../resources/README.md`](/Users/jason/Documents/SpaceCraftLivng/resources/README.md)
- [`../resources/images/README.md`](/Users/jason/Documents/SpaceCraftLivng/resources/images/README.md)
- [`../resources/audio/README.md`](/Users/jason/Documents/SpaceCraftLivng/resources/audio/README.md)
- [`../resources/videos/README.md`](/Users/jason/Documents/SpaceCraftLivng/resources/videos/README.md)

## 不建议作为当前执行依据

以下文档已归档，仅用于追溯背景：

- [`archive/2026-04-11/README.md`](/Users/jason/Documents/SpaceCraftLivng/docs/archive/2026-04-11/README.md)
- [`archive/`](/Users/jason/Documents/SpaceCraftLivng/docs/archive/)
