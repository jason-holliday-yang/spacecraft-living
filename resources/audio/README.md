...
- 当前资源目录额外包含 `10` 个已入库但尚未接线的新阶段 / 路线 BGM，详见“已入库未接线 BGM”
...
### `resources/audio/music/`（`24` 个，其中 `14` 个已接线，`10` 个未接线）
...
- `resources/audio/music/stages/`：`8` 个阶段 BGM，已入库未接线
- `resources/audio/music/routes/`：`2` 个路线 BGM，已入库未接线
...
### `resources/audio/music/stages/`（已入库未接线）
...
### `resources/audio/music/routes/`（已入库未接线）
...
当前运行时代码仍使用 `AudioScene` 按地图位置 / 场景语义切换 BGM。下一轮计划改为“阶段音乐主控”，详见 [`../../docs/plans/STAGE_MUSIC_PLAYBACK_PLAN.md`](/Users/jason/Documents/SpaceCraftLivng/docs/plans/STAGE_MUSIC_PLAYBACK_PLAN.md)。

在阶段化音乐改造完成前，下面仍是当前已接线代码事实：
...
- `music/stages/bgm_stage3_into_the_wild_*.ogg`
  目标用于 `Stage 3: Into the Wild / 走向荒野`
- `music/stages/bgm_stage4_rising_risk_*.ogg`
  目标用于 `Stage 4: Rising Risk / 风险升级`
- `music/stages/bgm_stage5_power_mire_*.ogg`
  目标用于 `Stage 5: Power Breakthrough / 动力突破`
- `music/stages/bgm_stage7_final_choice_*.ogg`
  目标用于 `Stage 7: Final Choice / 最终抉择`
- `music/routes/bgm_route_settlement_*.ogg`
  目标用于定居路线确认后的路线专属音乐
...
- [`src/game_play.c`](/Users/jason/Documents/SpaceCraftLivng/src/game_play.c)
  当前负责按地图地点与地表类型切换音乐场景，并触发脚步、采集、日志、修复、战斗、石碑与结局音效。阶段化音乐改造后，主 BGM 应改由 `Game_SelectMusicStage()` 选择。
...
- 阶段化 BGM 改造按 [`STAGE_MUSIC_PLAYBACK_PLAN.md`](/Users/jason/Documents/SpaceCraftLivng/docs/plans/STAGE_MUSIC_PLAYBACK_PLAN.md) 执行；代码完成前不要把 `music/stages/` 和 `music/routes/` 记为已接线。
