...
- 音频资源：
  当前代码主动加载 `33` 个运行时音频文件：`19` 个 `wav` 音效 + `14` 个 `ogg` 音乐文件；当前 `33/33` 全部存在。
- 视频资源：
...
- `resources/audio/music/`
  按场景拆分为 `menu / base / forest / swamp / ruins / boss / endings`
...
- 音频命名统一按运行时职责命名：
  ...
  - `music/<scene_group>/bgm_<scene_group>_<variant>.ogg` = 场景 BGM 变体
