...
- 图片资源：
  当前代码引用 `111` 张 PNG，目录中已存在 `108` 张，剩余 `3` 张为已接线但缺文件的 barrier 贴图。
- 叙事图片：
  当前已接线并存在 `48` 张，细分为 `5` 张开场分镜、`22` 张主线剧情图、`14` 张日志剧情图、`7` 张结局背景图。
- 地图 / 角色 / UI / 敌人图片：
  当前已接线并存在 `60` 张，覆盖角色、地图地块、场景物件、资源节点、制作图标、怪物与 Boss。
- 音频资源：
...
- 图片：
  开场分镜、`22` 张主线图、`14` 张日志图、`7` 张结局图、角色贴图、地图主地表、资源节点、图标、怪物贴图都已进入运行时加载链路。
- 音频：
